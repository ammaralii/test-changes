...
import com.test.changes.config.EmbeddedKafka;
import com.test.changes.config.KafkaSseConsumer;
import com.test.changes.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class TestChangesKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/test-changes-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/test-changes-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/test-changes-kafka/unregister"));
                return;
            }
        }
