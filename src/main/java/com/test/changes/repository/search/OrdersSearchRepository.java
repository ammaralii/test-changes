...
class OrdersSearchRepositoryInternalImpl implements OrdersSearchRepositoryInternal {
    ...
    OrdersSearchRepositoryInternalImpl(ElasticsearchRestTemplate elasticsearchTemplate, OrdersRepository repository) {
    ...
    public Page<Orders> search(Query query) {
        ...
        List<Orders> hits = searchHits.map(SearchHit::getContent).stream().collect(Collectors.toList());
