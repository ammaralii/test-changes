...
interface AddressesSearchRepositoryInternal {
    ...
    Page<Addresses> search(Query query);

    void index(Addresses entity);
}
...
class AddressesSearchRepositoryInternalImpl implements AddressesSearchRepositoryInternal {
    ...
    AddressesSearchRepositoryInternalImpl(ElasticsearchRestTemplate elasticsearchTemplate, AddressesRepository repository) {
    ...
    @Override
    public Page<Addresses> search(Query query) {
        SearchHits<Addresses> searchHits = elasticsearchTemplate.search(query, Addresses.class);
        List<Addresses> hits = searchHits.map(SearchHit::getContent).stream().collect(Collectors.toList());
        return new PageImpl<>(hits, query.getPageable(), searchHits.getTotalHits());
    }

    @Override
    public void index(Addresses entity) {
