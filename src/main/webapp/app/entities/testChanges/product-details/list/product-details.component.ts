...
import { EntityArrayResponseType, ProductDetailsService } from '../service/product-details.service';
...
export class ProductDetailsComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['description', 'imageUrl'];
  ...
  trackId = (_index: number, item: IProductDetails): number => this.productDetailsService.getProductDetailsIdentifier(item);

  search(query: string): void {
    if (query && ProductDetailsComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (ProductDetailsComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.productDetails = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IProductDetails[] | null): IProductDetails[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.productDetailsService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.productDetailsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
