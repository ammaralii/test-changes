...
export class OrdersComponent implements OnInit {
  ...
  trackId = (_index: number, item: IOrders): number => this.ordersService.getOrdersIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: IOrders[] | null): IOrders[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.ordersService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.ordersService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
