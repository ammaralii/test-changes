...
import { EntityArrayResponseType, CustomersService } from '../service/customers.service';
...
export class CustomersComponent implements OnInit {
  ...
  trackId = (_index: number, item: ICustomers): number => this.customersService.getCustomersIdentifier(item);

  search(query: string): void {
    if (query && CustomersComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (CustomersComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: ICustomers[] | null): ICustomers[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.customersService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.customersService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
