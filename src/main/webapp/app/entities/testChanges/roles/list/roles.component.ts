...
import { EntityArrayResponseType, RolesService } from '../service/roles.service';
...
export class RolesComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRoles): number => this.rolesService.getRolesIdentifier(item);

  search(query: string): void {
    if (query && RolesComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (RolesComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: IRoles[] | null): IRoles[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.rolesService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.rolesService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
