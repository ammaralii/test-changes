...
import { EntityArrayResponseType, OrderDetailsService } from '../service/order-details.service';
...
export class OrderDetailsComponent implements OnInit {
  ...
  trackId = (_index: number, item: IOrderDetails): number => this.orderDetailsService.getOrderDetailsIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.orderDetails = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IOrderDetails[] | null): IOrderDetails[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.orderDetailsService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.orderDetailsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
