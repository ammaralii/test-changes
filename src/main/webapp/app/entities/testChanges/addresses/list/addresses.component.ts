...
import { EntityArrayResponseType, AddressesService } from '../service/addresses.service';
...
export class AddressesComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['street', 'city', 'state'];
  ...
  trackId = (_index: number, item: IAddresses): number => this.addressesService.getAddressesIdentifier(item);

  search(query: string): void {
    if (query && AddressesComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (AddressesComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.addresses = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IAddresses[] | null): IAddresses[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.addressesService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.addressesService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
